...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/sec_xbrl_get_company_facts \
  -H 'Content-Type: application/json' \
  -d '{"cik":"320193"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/sec_xbrl_get_company_facts`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
